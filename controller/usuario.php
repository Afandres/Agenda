...
switch ($_GET["op"]){
	case 'guardaryeditar':
		...
		if (empty($idusuario)){
			$rspta=$usuario->insertar($nombre,$telefono,$email,$cargo,$login,$clave,$imagen,$_POST['permiso']);
			echo $rspta ? 1 : 2;
		}
		else {
			$rspta=$usuario->editar($idusuario,$nombre,$telefono,$email,$cargo,$login,$clave,$imagen,$_POST['permiso']);
			echo $rspta ? 3 : 4;
		}
	break;
